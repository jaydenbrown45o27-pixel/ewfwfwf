...
LocalPlayer.CharacterAdded:Connect(function(Character)
    if ButtonGui then ButtonGui:Destroy() ButtonGui = nil end
    for _, conn in pairs(InputConnections) do conn:Disconnect() end
    InputConnections = {}
end)

local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Evade" })
...
local MoveSec = PlayerTab:CreateSection("Movement", "Left")
...
local BhopSec = PlayerTab:CreateSection("Bhop", "Right")

BhopSec:AddToggle({
    Text = "Auto Bhop (hold space)",
    Default = false,
    Callback = function(Value)
        bhopEnabled = Value
    end,
...
local GravSec = PlayerTab:CreateSection("Gravity", "Left")

local gravitySlider = GravSec:AddSlider({
    Text = "Gravity",
    Min = 0,
    Max = 1000,
    Default = 50,
...
GravSec:AddButton({
    ...
    Callback = function()
        ...
        if gravitySlider then
            if gravitySlider.Set then gravitySlider:Set(50)
            elseif gravitySlider.SetValue then gravitySlider:SetValue(50) end
        end
    end,
})

local ReviveSec = AutoTab:CreateSection("Revive", "Right")
...
local EspSec = EspTab:CreateSection("ESP Controls", "Left")
...
local MiscSec = MiscTab:CreateSection("Visuals & Performance", "Left")
